...
func NewRootCmd() (*cobra.Command, params.EncodingConfig) {
	...
	cfg.SetAddressVerifier(chtTypes.VerifyAddressLen())
	cfg.Seal()
	...
	rootCmd := &cobra.Command{
		Use:   version.AppName,
		Short: "Chronic Token Daemon (server)",
...
func (ac appCreator) newApp(
	...
	var chtOpts []cht.Option
	if cast.ToBool(appOpts.Get("telemetry.enabled")) {
		chtOpts = append(chtOpts, chtKeeper.WithVMCacheMetrics(prometheus.DefaultRegisterer))
	}

	return app.NewChronicApp(logger, db, traceStore, true, skipUpgradeHeights,
...
func (ac appCreator) appExport(
	...
	loadLatest := height == -1
	var emptyChtOpts []cht.Option
	chtApp = app.NewChronicApp(
		logger,
		db,
		traceStore,
		loadLatest,
