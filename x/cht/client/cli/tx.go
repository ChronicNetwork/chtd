...
func GetTxCmd() *cobra.Command {
	txCmd := &cobra.Command{
		...
		Short:                      "cht transaction subcommands",
...
func parseStoreCodeArgs(file string, sender sdk.AccAddress, flags *flag.FlagSet) (types.MsgStoreCode, error) {
	...
	// gzip the cht file
	if ioutils.IsCht(cfile) {
		cfile, err = ioutils.GzipIt(cfile)

		if err != nil {
			return types.MsgStoreCode{}, err
		}
	} else if !ioutils.IsGzip(cfile) {
...
func parseInstantiateArgs(rawCodeID, initMsg string, sender sdk.AccAddress, flags *flag.FlagSet) (types.MsgInstantiateContract, error) {
	...
	if adminStr == "" && !noAdmin {
		return types.MsgInstantiateContract{}, fmt.Errorf("you must set an admin or explicitly pass --no-admin to make it immutible (chtd issue #719)")
	}
	if adminStr != "" && noAdmin {
