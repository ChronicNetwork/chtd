...
func TestUncompress(t *testing.T) {
	chtRaw, err := ioutil.ReadFile("../keeper/testdata/hackatom.cht")
	require.NoError(t, err)

	chtGzipped, err := ioutil.ReadFile("../keeper/testdata/hackatom.cht.gzip")
	require.NoError(t, err)
	...
	specs := map[string]struct {
		...
		"handle cht uncompressed": {
			src:       chtRaw,
			expResult: chtRaw,
		},
		"handle cht compressed": {
			src:       chtGzipped,
			expResult: chtRaw,
		},
		"handle nil slice": {
		...
		"handle incomplete gzip": {
			src:      chtGzipped[:len(chtGzipped)-5],
			expError: io.ErrUnexpectedEOF,
		},
